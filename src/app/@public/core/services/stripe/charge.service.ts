...
export class ChargeService extends ApiService{
  ...
  pay(payment: IPayment, stockChange: Array<IStock>) {
    return this.set(CREATE_PAY_ORDER, { payment, stockChange }).pipe(map((result: any) => {
      return result.chargeOrder;
    }));
  }

  listByCustomer(customer: string, limit: number, startingAfter: string, endingBefore: string) {
    return this.get(CHARGES_CUSTOMERS_LIST, { customer, limit, startingAfter, endingBefore }).pipe(map((result: any) => {
